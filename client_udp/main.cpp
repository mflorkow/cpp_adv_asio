...
/**
 * Retuns a input string from the shell.
...
startConnection ( std::string p_host_ , uint16_t p_port_ )
{
    ...
    boost::thread udpThread (boost::bind (&boost::asio::io_service::run , &udpClient::instance ().getIOService ()));
}
        ...
        try
        {
            ...
            } else
            {
                try
                {
                    ...
                    while (!udpClient::instance ().getIOService ().stopped ())
                    {
                        Console::instance ().processCommand (getInputString ());
                    }
                } catch (boost::exception & ec)
                {
                    std::cout << "An error in the connection appeared." << std::endl;
                }
            }
        } catch (std::exception)
        {
